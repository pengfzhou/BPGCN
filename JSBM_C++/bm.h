...
public:
	...
	//graph part
	int N,Nu,Ne, Q, Q_true, M, graph_max_degree,M1,M2;
	...
	vector <string> graph_ids;//it stores id (in string) of nodes.
        vector<int>pin_ids,randomindex;
	vector < vector <int> > graph_neis, graph_neis_inv, graph_edges1,graph_edges2,graph_edges;
	...
	vector < vector<int> > groups_true_nu,groups_true_ne,groups_true,groups_infer;
	...
	vector < double > na, nna, na_true, na_expect, nna_expect, eta, logeta, argmax_marginals; //nna is normalized na for degree corrected model.
        vector<double>ma,meta,ma_expect,logmeta;//
	vector < vector < double > > pab, cab, logcab, cab_true, pab_true, cab_expect;
        vector < vector < double > > qab, tab, logtab, tab_true, qab_true, tab_expect;
	int bm_dc;
	...
	double bp_last_diff;
	vector <double> field_iter, normtot_psi, pom_psi, exphi,expha, maxpom_psii_iter;
	...
	void graph_gen_graph(int,int, vector<double> , vector<double >,vector<double>);
	//block model
	blockmodel();
	void set_Q(int);
	void set_vflag(int);
	...
	void bm_init(vector<double>, vector<double>,vector<double>);
	...
	void output_marginals(string ,double,double,double,double);
