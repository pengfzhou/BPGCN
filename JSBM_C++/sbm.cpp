...
void parse_command_line(int argc, char **argv, double & rho,string & lfname, double &learning_rate, double &dumping_rate, double &bp_conv_crit, double &learning_conv_crit, int &time_conv, int &Q, int &randseed, int &learning_max_time,string& pastr, string& cabstr, int &infer_mode, string &eps_c_str, string &tabstr,int &init_flag, int &Nu,int &Ne, string &afname, int &vflag, string &gml_fname, string &spm_fname, string &cab_ifname,string &marginal_fname,bool &no_learn_pa,int &wblock, double &wstep, string &Aij_fname, int &spcmode, int &dc_flag, int &LARGE_DEGREE,string &cab_ofname, int &genmode)
{
...
//		{"brief",   no_argument,       &verbose_flag, 0},
	...
	int option_index = 0;
	char cc;
	while((cc=getopt_long(argc,argv,"l:r:R:e:b:E:t:q:D:v:s:L:c:a:I:T:p:P:i:m:n:w:W:v:d:f:M:A:h",long_options,&option_index))!=-1){
	  switch(cc){
			case 'l':	lfname=optarg;	break;
			case 'n':	Nu=atoi(optarg); break;
                        case 'b':       Ne=atoi(optarg);break;
			...
			case 'p':	pastr=optarg; break;
                        case 'f':       rho=atof(optarg);break;
                        case 'a':       tabstr=optarg; break;   
...
int main(int argc, char** argv)
{
	...
	double learning_rate=1.0,dumping_rate=1.0,bp_conv_crit=1.0e-3,learning_conv_crit=5.0e-8,wstep=0.02,rho=0.0;
        double eps,eps1,c,c1;
	long t_begin=get_cpu_time();
	int N=-1, Nu=-1,Ne=-1,time_conv=100,Q=2,randseed=0,learning_max_time=1000,infer_mode=1,init_flag=1,vflag=0,wblock=200,spcmode=3,LARGE_DEGREE=50,genmode=0;
	...
	string lfname,pastr,cabstr,tabstr,eps_c_str,gml_fname("tmp.gml"),afname,spm_fname,cab_ifname,cab_ofname,marginal_fname,Aij_fname;
	char **myargv;
	...
	parse_command_line(argc, argv, rho,lfname, learning_rate, dumping_rate, bp_conv_crit, learning_conv_crit, time_conv, Q, randseed, learning_max_time, pastr, cabstr, infer_mode, eps_c_str, tabstr,init_flag, Nu,Ne, afname, vflag,gml_fname,spm_fname,cab_ifname,marginal_fname,no_learn_pa,wblock,wstep,Aij_fname,spcmode,dc_flag,LARGE_DEGREE,cab_ofname,genmode);
	...
	vector<double> pa, cab,tab;
	if(!cab_ifname.empty()){ // set pa and cab from file named cab_ifname.
		read_cab_from_file(cab_ifname,pa,cab,Q);
	}else if(!eps_c_str.empty() && !tabstr.empty() ){ //epsilon,c mode. Set by -P parameter
		cout<<1<<endl;
		assert(genmode == 0 || genmode ==1|| genmode ==2);
		assert(Nu != -1 && "you need to specify Nu by -n parameter!");
                assert(Ne != -1 && "you need to specify Ne by -b parameter!");
		vector<string> tmp_eps_c=strsplit(eps_c_str,",");
                vector<string> tmp_tab=strsplit(tabstr,",");
		assert(tmp_eps_c.size() == 2);
                assert(tmp_tab.size() == 2);
		 eps=atof(tmp_eps_c[0].c_str());
                 eps1=atof(tmp_tab[0].c_str());
		 c=atof(tmp_eps_c[1].c_str());
                c1=atof(tmp_tab[1].c_str());
		assert(c>0.0);
                assert(c1>0.0);
		double cin=0,co=0;
                double tin=0,to=0;
		...
		if(genmode==0){
			if(eps<0 || eps>99){
				...
				double eta=1.0/Q;
				cin=c/(Q*(Q-1)*eps*eta*eta+Q*eta*(eta*(eta*Nu-1)/Nu/eta));
				co=eps*cin;
			}
			...
			for(unsigned int q=0;q<Q;q++){
				if(dc_flag==1) tab.push_back(tin);
				else if(dc_flag==2) tab.push_back(tin/c1/c1);
				else tab.push_back(tin);
				for(int t=q+1;t<Q;t++) {
					if(dc_flag==1) tab.push_back(to);
					else if(dc_flag==2) tab.push_back(to/c1/c1);
					else tab.push_back(to);
				}
			}

		}
                 // initialize tab for bi graph
                        
		
		cout<<"eps_c mode: eps="<<eps<<" c="<<c<<" cin="<<cin<<" cout="<<co<<" dc_flag="<<dc_flag<<endl;
                cout<<"cab mode: eps1="<<eps1<<" c1="<<c1<<" tin="<<tin<<" tout="<<to<<" dc_flag="<<dc_flag<<endl;
	}
	else{
		bm_init_random=true;
	}
	...
	if(function == string("gen")){ // generation
		bm.graph_gen_graph(Nu,Ne, pa, cab,tab);
		...
		//bm.bm_allocate();
		//bm.bm_init(pa,cab,tab);
                bm.graph_gen_graph(Nu,Ne, pa, cab,tab);
                
                bm.chos_pin_node(rho);
                
		bm.do_inference(infer_mode, init_flag,bp_conv_crit, time_conv, dumping_rate);
		if(!marginal_fname.empty()) bm.output_marginals(marginal_fname,eps,eps1,c,c1);
	}
	else{
		show_help(argv);
	}
